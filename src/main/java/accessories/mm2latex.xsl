...
<xsl:stylesheet
    ...
    <xsl:template match="node" mode="heading">
        ...
        <xsl:choose> <!-- we change our mind if the NoHeading attribute is present -->
            ...
            <xsl:otherwise>
                ...
                <xsl:value-of select="string($command)"/>
                <xsl:text>{</xsl:text>
    ...
    <xsl:template match="node">
        <xsl:variable name="nodesNextLevel" select="count(./node)"/>
        <!-- Anzahl unterlagerter Knoten feststellen -->
        <xsl:variable name="nodesThisLevel" select="count(../node) - count(../node//img) - count(../node//table)"/>
        <!-- Anzahl Knoten in diesem Level feststellen -->
        ...
        <!-- Absatz endet immer nach einer node -->
        ...
        <xsl:if test="node/arrowlink">
            <xsl:text>siehe auch \ref{</xsl:text>
            <xsl:value-of select="node/arrowlink/@destination"/>
            <xsl:text>}</xsl:text>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="hook|@LINK">
        <xsl:text>Siehe auch \url{</xsl:text>
        <xsl:value-of select="."/>
        <xsl:text>}</xsl:text>
    </xsl:template>
...
-->
    ...
    <xsl:template match="img" mode="richcontent">
        <xsl:param name="id"/>
        <!-- id for label -->
        <xsl:param name="format" select="'scale=0.5'"/>
        <!-- Formatting -->
        <xsl:param name="caption" select="'No Caption defined in Mindmap'"/>
        <!-- Formatting -->

...
<xsl:stylesheet version="1.0"
            ...
            <head>
                ...
                <xsl:element name="link">
                    ...
                    <xsl:attribute name="href">
                        <xsl:value-of select="$destination_dir"/>freemind2html.css
                    </xsl:attribute>
                    <xsl:attribute name="type">text/css</xsl:attribute>
                </xsl:element>
                <meta name="generator"
                      content="FreeMind-XSL Stylesheet (see: http://freemind-xsl.dev.slash-me.net/ for details)"/>
                <xsl:element name="script">
                    <xsl:attribute name="type">text/javascript</xsl:attribute>
                    <xsl:attribute name="src">
                        <xsl:value-of select="$destination_dir"/>freemind2html.js
                    </xsl:attribute>
                    &#160;
                </xsl:element>
            ...
            <body>
                ...
                <div style="width:96%; 	padding:2%; 	margin-bottom:10px; 	border: 0px; 	text-align:center; 	vertical-align:center;">
                    <xsl:element name="img">
                        <xsl:attribute name="src">
                            <xsl:value-of select="$destination_dir"/>image.png
                        </xsl:attribute>
                        <xsl:attribute name="style">margin-bottom:10px; border: 0px; text-align:center;
        ...
        <div>
                    ...
                    <xsl:choose>
                        <xsl:when test="cloud/@COLOR">
                            ...
                            <xsl:attribute name="style">background-color:
                                <xsl:value-of select="cloud/@COLOR"/>
                            </xsl:attribute>
                        </xsl:when>
            ...
            <xsl:choose>
                <xsl:when test="node">
                    ...
                    <xsl:element name="img">
                        <xsl:attribute name="src">
                            <xsl:value-of select="$destination_dir"/>hide.png
                        </xsl:attribute>
                        <xsl:attribute name="class">hideshow</xsl:attribute>
                        <xsl:attribute name="alt">hide</xsl:attribute>
                        <xsl:attribute name="onClick"><![CDATA[toggle("]]><xsl:value-of select="$contentID"/>
                            <![CDATA[")]]>
                        </xsl:attribute>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:element name="img">
                        <xsl:attribute name="src">
                            <xsl:value-of select="$destination_dir"/>leaf.png
                        </xsl:attribute>
                        <xsl:attribute name="class">hideshow</xsl:attribute>
            ...
            <xsl:if test="@ID">
                ...
                <a>
                    <xsl:attribute name="id">FM<xsl:value-of select="@ID"/>FM
                    </xsl:attribute>
                </a>
            </xsl:if>
    ...
    <xsl:template match="font">
        <xsl:if test="string-length(@SIZE) > 0">font-size:<xsl:value-of select="round((number(@SIZE) div 12)*100)"/>%;
        </xsl:if>
        ...
        <xsl:element name="div">
            ...
            <xsl:if test="@COLOR or @BACKGROUND_COLOR or font">
                <xsl:attribute name="style">
                    <xsl:if test="@COLOR">color:<xsl:value-of select="@COLOR"/>;
                    </xsl:if>
                    <xsl:if test="@BACKGROUND_COLOR">background-color:<xsl:value-of select="@BACKGROUND_COLOR"/>;
                    </xsl:if>
            ...
            <xsl:if test='hook/@NAME="plugins/map/MapNodePositionHolder.properties"'>
				...
				-->
                            <xsl:when
                                    test="hook/Parameters/@XML_STORAGE_TILE_SOURCE='org.openstreetmap.gui.jmapviewer.tilesources.OsmTileSource$Mapnik'">
                                M
                            </xsl:when>
                            <!--
                                            -->
                            <xsl:when
                                    test="hook/Parameters/@XML_STORAGE_TILE_SOURCE='plugins.map.FreeMindMapController$TransportMap'">
                                T
                            </xsl:when>
                            <!--
                                            -->
                            <xsl:when
                                    test="hook/Parameters/@XML_STORAGE_TILE_SOURCE='org.openstreetmap.gui.jmapviewer.tilesources.OsmTileSource$CycleMap'">
                                C
                            </xsl:when>
                            <!--
                                            -->
                            <xsl:when
                                    test="hook/Parameters/@XML_STORAGE_TILE_SOURCE='plugins.map.FreeMindMapController$MapQuestOpenMap'">
                                Q
                            </xsl:when>
                            <!--
                                            -->
                    ...
                    <xsl:element name="img">
                        <xsl:attribute name="src"><xsl:value-of select="$destination_dir"/>map_location.png
                        </xsl:attribute>
            ...
            <xsl:when test="not($show_link_url='true')">
                <xsl:variable name="link">
                    <xsl:choose>
                        ...
                        <xsl:when test="starts-with(@LINK, '#')">#FM<xsl:value-of select="substring(@LINK,2)"/>FM
                        </xsl:when>
                        <xsl:otherwise>
                ...
                <xsl:if test="not($show_icons='false')">
                    ...
                    <xsl:element name="a">
                        ...
                        <xsl:element name="img">
                            <xsl:attribute name="src"><xsl:value-of select="$destination_dir"/>ilink.png
                            </xsl:attribute>
    ...
    <xsl:template name="output-arrowlinks">
        ...
        <xsl:for-each select="arrowlink">
            <xsl:text></xsl:text>
            <a>
                <xsl:attribute name="onclick">getVisibleParents('FM<xsl:value-of select="@DESTINATION"/>FM')
                </xsl:attribute>
                <xsl:attribute name="href">#FM<xsl:value-of select="@DESTINATION"/>FM
                </xsl:attribute>
                <xsl:choose>
    ...
    <xsl:template name="output-icons">
        <xsl:if test="not($show_icons='false')">
            <xsl:for-each select="icon">
                <xsl:element name="img">
                    <xsl:attribute name="src">
                        <xsl:value-of select="$destination_dir"/>icons/<xsl:value-of select="@BUILTIN"/>.png
                    </xsl:attribute>

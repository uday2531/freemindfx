...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        ...
        <html>&#xA;
            <head>&#xA;
                <title><xsl:value-of select="map/node/@TEXT"/>//mm2html.xsl FreemindVersion:
            ...
            <body>
                ...
                <p class="info">
                    <xsl:value-of select="map/node/@TEXT"/>//mm2html.xsl FreemindVersion:
                    <xsl:value-of select="$mapversion"/>
                    &#xA;
                </p>
            </body>
                    ...
                    <a>
                        <xsl:attribute name="style">
                            <xsl:if test="$thiscolor != ''">
                                ...
                                <xsl:value-of select="$thiscolor"/>
                                <xsl:text>;</xsl:text>
                            </xsl:if>
                            <xsl:if test="$fontface != ''">
                                <xsl:text>font-family:</xsl:text>
                                <xsl:value-of select="translate($fontface,$ucletters,$lcletters)"/>
                                <xsl:text>;</xsl:text>
                            </xsl:if>
